...
if hyp_key == '':
    ...
    with open('hyp.json', 'r') as file: 
        ...
        # Convert To Bool
        hyp['gmi+xm']['use_xm'] = hyp['gmi+xm']['use_xm'] == 'True'
        hyp['gmi']['use_xm'] = hyp['gmi']['use_xm'] == 'True'
...
for run_idx in tqdm(range(run_count)):
    ...
    for d in dimensions: 
        ...
        # GMI-ID
        if use_id:
            gmi_id = GMIEmbedding(graph = graph, 
                       embed_dim = d, 
                       feature_matrix = np.identity(len(graph)), 
                       use_xm = hyp['gmi']['use_xm'], 
                       ortho_ = hyp['gmi']['ortho'], 
                       sparse_ = hyp['gmi']['sparse'], 
                       batch_size = 1, 
                       model_name = model_name)
            embed_id = gmi_id.get_embedding()
        ...
        try:
            ...
            if use_id:
                ...
                results[d]['gmi_id_time'].append(gmi_id.time_per_epoch)
            ...
            if use_id:
                ...
                results[d]['gmi_id_time'] = [gmi_id.time_per_epoch]
